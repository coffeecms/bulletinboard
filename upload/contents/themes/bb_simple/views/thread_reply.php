    ...
    <main>
      ...
      if($total > 0)
      {
        ...
        for ($i=0; $i < $total; $i++) { 
          ...
          $li.='<li class="breadcrumb-item"><a href="'.thread_url($thread_data[0]['friendly_url']).'">'.$thread_data[0]['title'].'</a></li>';
        echo '
              ...
              <?php load_hook('bb_top_write_thread_tools');?>
                <?php BB_PHPCode::load('bb_top_write_thread_tools');?>
                        ...
                        <p style='border: 1px solid #cbcbcb;'>
                            <?php if(isset($ref_post_id[5])){ ?>
                            <textarea id="editor" rows="15" name="send[content]" class="form-control new-post-content ckeditor"><blockquote class="quote"><a href="<?php echo post_url($thread_friendly_url,$ref_post_id);?>"><?php echo $ref_post_username;?> said:</a><br><?php echo $ref_post_content;?></blockquote><br><br></textarea>
                            <?php }else{ ?>
    ...
    <div style="margin:0px;padding:0px;min-width: 280px;min-height:90px;max-width: 280px;max-height:140px;background-color: #2885BF;color:#ffffff;border-radius: 3px;padding:3px;position: relative;">
                ...
                <?php } ?>   
                  ...
                  <div class='row margin-top-20'>
                    ...
                    <button type='button' class='btn btn-primary btnSubmit'><i class='fas fa-paper-plane'></i> Send reply</button>
                    </div>                    
                  </div>
                  <!-- row -->                
...
var thread_title='<?php echo $thread_data[0]['title'];?>';
...
pageData['attach_files_upload_status']=0;
  ...
  $(document).on('click','.btnSubmit',function(){
    ...
    var jsonData={};
      ...
      jsonData['thread_title']=thread_title;
