    ...
    <main>
      ...
      if($total > 0)
      {
        ...
        for ($i=0; $i < $total; $i++) { 
          ...
          $li.='<li class="breadcrumb-item"><a href="'.thread_url($post_data[0]['friendly_url']).'">'.$post_data[0]['title'].'</a></li>';
        echo '
                ...
                <?php BB_PHPCode::load('bb_top_write_thread_tools');?>
                      <div class="input-group mb-3" >
                            ...
                            <?php 
                              ...
                              for ($i=0; $i < $total; $i++) { 
                                if(Configs::$_['list_post_prefix'][$i]['prefix_id']==$post_data[0]["prefix_id"])
                                {
                                    $li.='<option selected value="'.Configs::$_['list_post_prefix'][$i]['prefix_id'].'">'.Configs::$_['list_post_prefix'][$i]['title'].'</option>';
                                }
                                else
                                {
                            ...
                            <input type="text" class="form-control new-title" value="<?php echo $post_data[0]['title'];?>" placeholder="Title" aria-label="Title" aria-describedby="basic-addon1">
                        </div>
                ...
                <div class="tab-content" id="pills-tabContent">
                    <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                      ...
                      <p>
                        ...
                        <p>
                          ...
                          <input type="text" class="form-control new-tags" aria-label="Tags" value="<?php echo $post_data[0]['tags'];?>" aria-describedby="basic-addon1">
                        </div>
                        ...
                        <p>
                          ...
                          <input type="text" class="form-control new-keywords" aria-label="Keywords" value="<?php echo $post_data[0]['keywords'];?>" aria-describedby="basic-addon1">
                        </div>

                        </p>
                        <?php if(bb_forum_has_permission($forum_id,'BB20010')==false && bb_forum_has_permission($forum_id,'BB10010')==true){ ?>           
                        <div class="input-group mb-3">
                            <?php if((int)$post_data[0]['is_stick']==1){ ?>
                                <button type="button" class="btn btn-primary pin_thread btn-success" data-checked="0" style=""><i class="fas fa-check-square"></i></button>
                            <?php }else{ ?>
                        ...
                        <div class="input-group mb-3">
                            <?php if((int)$post_data[0]['status']==1){ ?>
                                <button type="button" class="btn btn-primary status_thread btn-success" data-checked="0" style=""><i class="fas fa-check-square"></i></button>
                            <?php }else{ ?>
                                <button type="button" class="btn btn-primary status_thread btn-none" data-checked="0" style=""><i class="fas fa-square"></i></button>
                            <?php } ?>
                            <span class="input-group-text">Activate this thread</span>
                          </div>

                          <div class="input-group mb-3">

                          <?php if((int)$post_data[0]['allow_reply']==1){ ?>
                          <button type="button" class="btn btn-primary disallow_reply btn-none"  data-checked="0" style=""><i class="fas fa-square"></i></button>
                          <?php }else{ ?>
                            <button type="button" class="btn btn-primary disallow_reply btn-success"  data-checked="0" style=""><i class="fas fa-check-square"></i></button>
                            <?php } ?>
                    ...
                    <div class="tab-pane fade" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab">
                        ...
                        <div class="input-group mb-3">
                            ...
                            <input type="text" class="form-control add_poll_question" value="<?php if(count($post_poll_data) > 0){ echo $post_poll_data[0]['question'];}?>" aria-label="Question" aria-describedby="basic-addon1">
                        </div>
    ...
    <div style="margin:0px;padding:0px;min-width: 280px;min-height:90px;max-width: 280px;max-height:140px;background-color: #2885BF;color:#ffffff;border-radius: 3px;padding:3px;position: relative;">
                ...
                <?php } ?>
                  ...
                  <div class='row margin-top-20'>
                    ...
                    <button type='button' class='btn btn-primary btnSubmit' style='float:left;'><i class='fas fa-paper-plane'></i> Save changes</button>
                    <button type='button' class='btn btn-danger btnDeleteThread' style='float:right;'><i class='fas fa-trash'></i> Delete this thread</button>
                    </div>                    
                  </div>
                  <!-- row -->                
...
pageData['post_poll_data']=<?php echo json_encode($post_poll_data);?>;
pageData['post_poll_answer_data']=<?php echo json_encode($post_poll_answer_data);?>;
pageData['post_attach_files_data']=<?php echo json_encode($post_attach_files_data);?>;
pageData['thread_id']='<?php echo $thread_id;?>';
pageData['poll_id']='<?php echo $poll_id;?>';
pageData['author_id']='<?php echo $post_data[0]['user_id'];?>';
pageData['attach_files_upload_status']=0;
pageData['forum_data']=<?php echo json_encode($forum_data)?>;
pageData['forum_url']='<?php echo forum_url($forum_data[0]['friendly_url'])?>';
pageData['thread_url']='<?php echo thread_url($post_data[0]['friendly_url'])?>';
...
$(document).on('click','.new_attach_files',function(){
  ...
  function prepareNewAttachFilesData()
  {
    ...
    console.log(pageData['attach_files']);
...
function prepareAttachFilesData()
{
  ...
  for (let i = 0; i < total; i++) {
    li+='<div class="input-group mb-3">';
    li+='<button type="button" title="Delete this file" class="btn btn-danger old_attach_files old_attach_files_'+pageData['post_attach_files_data'][i]['file_id']+' " data-path="'+pageData['post_attach_files_data'][i]['file_path']+'" data-id="'+pageData['post_attach_files_data'][i]['file_id']+'" data-checked="0" style=""><i class="fas fa-trash-alt"></i></button>';
...
{
    var total=pageData['post_poll_answer_data'].length;
    ...
    total_poll_answer=total-1;

    for (let i = 0; i < total; i++) {
      if(typeof pageData['post_poll_answer_data'][i]=='undefined')
      {
        continue;
      }
        ...
        li+='<input type="text" class="form-control answer_poll" data-id="'+pageData['post_poll_answer_data'][i]['answer_id']+'" value="'+pageData['post_poll_answer_data'][i]['content']+'" aria-label="Answer" aria-describedby="basic-addon1">';
        li+='</div>';       
    }

    li+='<div class="input-group mb-3">';
    li+='<span class="input-group-text" id="basic-addon1">Answer '+total_poll_answer.toString()+'</span>';
    li+='<input type="text" class="form-control answer_poll" data-id="" value="" aria-label="Answer" aria-describedby="basic-addon1">';
    li+='</div>';   
    
    $('.wrap_list_poll_answer').html(li);

    if(pageData['post_poll_data'].length > 0)
    {
        if(pageData['post_poll_data'][0]['poll_choice']=='unique')
        {
            $('.poll_choice_unique').trigger('click');
        }
        if(pageData['post_poll_data'][0]['poll_choice']=='multi')
        {
            $('.poll_choice_multi_unique').trigger('click');
        }
        if(pageData['post_poll_data'][0]['poll_choice']=='max')
        {
            $('.poll_choice_max').trigger('click');
        }

        // poll_choice_max_number
        $('.poll_choice_max_number').val(pageData['post_poll_data'][0]['poll_choice_max']);

        if(parseInt(pageData['post_poll_data'][0]['allow_change_answer'])==1)
        {
            $('.poll_option_1').trigger('click');
        }
        if(parseInt(pageData['post_poll_data'][0]['allow_guest_access'])==1)
        {
            $('.poll_option_2').trigger('click');
        }
        if(parseInt(pageData['post_poll_data'][0]['set_close_poll_day'])==1)
        {
            $('.poll_option_3').trigger('click');
        }
    }
}
    ...
    $(document).on('click','.old_attach_files',function(){
        ...
        if(confirm('Are you want to delete this file ?'))
        {
          ...
          postData(API_URL+'plugin_api?plugin=bulletinboard&func=frontend_api&api_nm=bb_delete_attach_files', jsonData).then(data => {
            // console.log(data); // JSON data parsed by `data.json()` call
    ...
    $(document).on('keyup','.answer_poll',function(){
      ...
      if(theVal.length > 0)
      {
        ...
        li+='<div class="input-group mb-3">';
        li+='<span class="input-group-text" id="basic-addon1">Answer '+total_poll_answer.toString()+'</span>';
        li+='<input type="text" class="form-control answer_poll" data-id="" aria-label="Answer" aria-describedby="basic-addon1">';
        li+='</div>';
  ...
  $(document).on('click','.btnDeleteThread',function(){
      if(confirm('Are you want to delete this thread ?'))
      {
        ...
        jsonData['author_id']=pageData['author_id'];
  ...
  $(document).on('click','.status_thread',function(){
  ...
  $(document).on('click','.btnSubmit',function(){
    ...
    var jsonData={};
      ...
      jsonData['status_thread']='0';
      jsonData['poll_id']=pageData['poll_id'];
      ...
      if($('.status_thread').length > 0)
      {
        if($('.status_thread').hasClass('btn-success'))
        {
          jsonData['status_thread']='1';
        }
      }
      ...
      jsonData['friendly_url']=pageData['thread_url'];
      ...
      $('.answer_poll').each(function(){
        if($(this).val().trim().length > 0)
        {
          ...
          jsonData['poll_answer_id']+=$(this).attr('data-id')+'|||';
        }
      });
      ...
      postData(API_URL+'plugin_api?plugin=bulletinboard&func=frontend_api&api_nm=bb_edit_thread', jsonData).then(data => {
        // console.log(data); // JSON data parsed by `data.json()` call
        if(data['data'].indexOf('http')>=0)
        {
          location.href=pageData['thread_url'];
        }
        else
        {
          showAlert('',data['data']);
