...
if(!isset(Configs::$_['bb_license_key'][5]))
{
    redirect_to(SITE_URL.'admin/plugin_page_url?plugin=bulletinboard&page=active_system');
        ...
        $queryStr.="select a.*,b.subject as title,b.username as author ";
        $queryStr.=" from bb_message_ads_data as a join bb_message_data as b ON a.message_id=b.message_id ";
...
<div class="modal fade" id="modalAdd"  style='z-index:99999;' data-backdrop="false" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body ">
      <p>
        <label><strong><?php echo get_text_by_lang('Subject','admin');?></strong></label>
        <input type="text" class="form-control add-subject input-size-medium" name="send[keywords]" placeholder="Subject" />
    </p> 
      ...
      <div class="container-fluid">
        ...
        <div class="row">
              ...
              <div class="card-header border-0">
                <h3 class="card-title"><?php echo get_text_by_lang('Message Ads','admin');?></h3>
              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-striped table-valign-middle">
                  ...
                  <tr>
                    ...
                    <th><?php echo get_text_by_lang('Create Time','admin');?></th>
                  </tr>
                  </thead>
...
function prepareShowPost()
{
  ...
  for (var i = 0; i < total; i++) {
    ...
    li+='<td>'+moment(pageData['listPost'][i]['ent_dt'],'YYYY-MM-DD HH:mm:ss').format('MM/DD/YYYY HH:mm:ss')+'</td>';
...
$(document).on('click','.btnCreateAds',function(){
    ...
    sendData['type']='1';
    sendData['subject']=$('.add-subject').val().trim();
    sendData['content']=CKEDITOR.instances.editor.getData();

    if(sendData['subject'].length==0)
    {
        showAlert('','Subject not allow blank!');
        return;
    }

    postData(API_URL+'plugin_api?plugin=bulletinboard&func=frontend_api&api_nm=bb_add_message_ads', sendData).then(data => {
    // console.log(data); // JSON data parsed by `data.json()` call
    $('#modalAdd').modal('hide');
...
$(document).on('click','.btnApply',function(){
  ...
  postData(API_URL+'plugin_api?plugin=bulletinboard&func=frontend_api&api_nm=bb_message_ads_action_apply', sendData).then(data => {
    // console.log(data); // JSON data parsed by `data.json()` call
...
$(document).on('click','.btnPrev',function(){
  ...
  postData(API_URL+'plugin_api?plugin=bulletinboard&func=frontend_api&api_nm=bb_get_list_message_ads', sendData).then(data => {
    // console.log(data); // JSON data parsed by `data.json()` call
    $('#modalSearch').modal('hide');
