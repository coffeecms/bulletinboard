...
if(!isset(Configs::$_['user_permissions']['BB30021']))
{
  redirect_to(SITE_URL.'admin/notfound');
    ...
    $listData=$db->query("select * from bb_banned_browser_data order by ent_dt desc");
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><?php echo get_text_by_lang('Add new browser','admin');?></h5>
      ...
      <div class="modal-body">
        ...
        <div class="col-lg-12">
                        ...
                        <p>
                            <label><strong><?php echo get_text_by_lang('Browser name','admin');?></strong></label>
                            <input type="text" class="form-control input-size-medium add-browser" name="send[title]" placeholder="<?php echo get_text_by_lang('Browser name','admin');?>" id="txtTitle" />
                        </p>
      ...
      <div class="container-fluid">
        ...
        <div class="row">
              ...
              <div class="card-header border-0">
                <h3 class="card-title"><?php echo get_text_by_lang('Banned Browser','admin');?></h3>
              ...
              <table class="table table-hover table-striped table-valign-middle">
                  <thead>
                    <tr >
                                              ...
                                              <th><?php echo get_text_by_lang('Browser name','admin');?></th>
...
function prepareShowData()
{
  ...
  for (var i = 0; i < total; i++) {
    ...
    td+='<tr class="tr-id-'+pageData['listData'][i]['browser_name']+'" style="cursor:pointer;">';
    td+='<td style="width:80px;text-align: left;">';
    td+='<button type="button" class="btn btn-default btn-xs btn-checkbox" data-checked="no" data-id="'+pageData['listData'][i]['browser_name']+'"><i class="fas fa-square"></i></button>';
    td+='</td>';
    td+='<td class="td-category" data-id="'+pageData['listData'][i]['browser_name']+'">'+pageData['listData'][i]['browser_name'];
    td+='</td>';
    td+='<td class="td-category text-right" data-id="'+pageData['listData'][i]['browser_name']+'">'+pageData['listData'][i]['ent_dt'];
    td+='</td>';
  ...
  $('.list-data').html(td);
    ...
    $(document).on('click','.btnSaveAdd',function(){
      ...
      sendData['browser']=$('.add-browser').val().trim();
   
      sendData['type']='1';

        
      if(sendData['browser'].length==0)
      {
        showAlert('','Browser name not allow is blank');return false;
      }

      postData(API_URL+'plugin_api?plugin=bulletinboard&func=bb_add_banned_browser', sendData).then(data => {
        console.log(data); // JSON data parsed by `data.json()` call
        // console.log(data['error']);
        $('.txtThumbnail').val('');
        location.reload();
...
$(document).on('click','.btnApply',function(){
  ...
  sendData['list_browser']=pageData['list_browser'];
  pageData['action']=$('.post-action > option:selected').val().trim();
  sendData['action']=pageData['action'];

  postData(API_URL+'plugin_api?plugin=bulletinboard&func=bb_banned_browser_action_apply', sendData).then(data => {
    // console.log(data); // JSON data parsed by `data.json()` call
