...
if(!isset(Configs::$_['user_permissions']['BB30021']))
{
  redirect_to(SITE_URL.'admin/notfound');
    ...
    $listData=$db->query("select * from bb_banned_os_data order by ent_dt desc");
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="col-lg-12">
                        ...
                        <p>
                            <label><strong><?php echo get_text_by_lang('OS name','admin');?></strong></label>
                            <input type="text" class="form-control input-size-medium add-os" name="send[title]" placeholder="<?php echo get_text_by_lang('OS name','admin');?>" id="txtTitle" />
                        </p>
      ...
      <div class="container-fluid">
        ...
        <div class="row">
              ...
              <div class="card-header border-0">
                <h3 class="card-title"><?php echo get_text_by_lang('Banned operating system','admin');?></h3>
              ...
              <table class="table table-hover table-striped table-valign-middle">
                  <thead>
                    <tr >
                                              ...
                                              <th><?php echo get_text_by_lang('Operating system','admin');?></th>
...
function prepareShowData()
{
  ...
  for (var i = 0; i < total; i++) {
    ...
    td+='<tr class="tr-id-'+pageData['listData'][i]['os_name']+'" style="cursor:pointer;">';
    td+='<td style="width:80px;text-align: left;">';
    td+='<button type="button" class="btn btn-default btn-xs btn-checkbox" data-checked="no" data-id="'+pageData['listData'][i]['os_name']+'"><i class="fas fa-square"></i></button>';
    td+='</td>';
    td+='<td class="td-category" data-id="'+pageData['listData'][i]['os_name']+'">'+pageData['listData'][i]['os_name'];
    td+='</td>';
    td+='<td class="td-category text-right" data-id="'+pageData['listData'][i]['os_name']+'">'+pageData['listData'][i]['ent_dt'];
    td+='</td>';
  ...
  $('.list-data').html(td);
    ...
    $(document).on('click','.btnSaveAdd',function(){
      ...
      sendData['os']=$('.add-os').val().trim();
   
      sendData['type']='1';

        
      if(sendData['os'].length==0)
      {
        showAlert('','OS name not allow is blank');return false;
      }


      postData(API_URL+'plugin_api?plugin=bulletinboard&func=bb_add_banned_os', sendData).then(data => {
        console.log(data); // JSON data parsed by `data.json()` call
        // console.log(data['error']);
        $('.txtThumbnail').val('');
        location.reload();
...
$(document).on('click','.btnApply',function(){
  ...
  sendData['list_os']=pageData['list_os'];
  pageData['action']=$('.post-action > option:selected').val().trim();
  sendData['action']=pageData['action'];

  postData(API_URL+'plugin_api?plugin=bulletinboard&func=bb_banned_os_action_apply', sendData).then(data => {
    // console.log(data); // JSON data parsed by `data.json()` call
